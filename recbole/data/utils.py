...
def data_preparation(config, dataset):
    ...
    if dataloaders is not None:
        ...
        g1_user_list = train_dataset.g1_user_list
        g2_user_list = train_dataset.g2_user_list
    ...
    return train_data, valid_data, test_data, g1_user_list, g2_user_list
...
def split_user_group(uid_in_interaction, dataset_name):
    # 转为df结构
    ...
    print('inter_num_res',inter_num_res)
    ...
    active_num = int(all_num * 0.05)
    active_list = df.loc[:active_num - 1, 'uid']
    active_list = active_list.tolist()
    inactive_list = df.loc[active_num:, 'uid']
    inactive_list = inactive_list.tolist()
    print('len(active_list)',len(active_list))
    print('len(inactive_list)',len(inactive_list))
    # print('active_list',active_list)
    # print('inactive_list',inactive_list)


    # 保存
    #save_path = '.../re-rank/data/{}/'.format(dataset_name)
    #os.makedirs(save_path, exist_ok=True)
    #np.savetxt('active_list.txt', active_list)
    #np.savetxt('inactive_list.txt', inactive_list)


    '''
    # 加载
    active_list = np.loadtxt('active_list.txt')
    active_list=active_list.tolist()
    inactive_list = np.loadtxt('inactive_list.txt')
    inactive_list=inactive_list.tolist()
    print('active_list',type(active_list))
    print('inactive_list', type(inactive_list))
    '''
