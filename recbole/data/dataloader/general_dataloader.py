...
class NegSampleEvalDataLoader(NegSampleDataLoader):
    ...
    def _next_batch_data(self):
        if self.neg_sample_args['strategy'] == 'by':
            ...
            for idx, uid in enumerate(uid_list):
                ...
                data_list.append(self._neg_sampling(self.dataset[index]))  # 对应！！！
    ...
    def _new_neg_sampling(self, inter_feat):  # 新采样方式！
        user_ids = inter_feat[self.uid_field].numpy()[0:1]
        item_ids = inter_feat[self.iid_field].numpy()[0:1]
        neg_item_ids = self.sampler.sample_by_user_ids(user_ids, item_ids, self.neg_sample_num)

        pos_inter_num = len(inter_feat)
        new_data = {}
        new_data_size = (pos_inter_num + self.neg_sample_num,)
        new_data[self.uid_field] = torch.full(new_data_size, inter_feat[self.uid_field][0])
        new_data[self.iid_field] = torch.zeros(new_data_size, dtype=inter_feat[self.iid_field].dtype)
        new_data[self.iid_field][:pos_inter_num] = inter_feat[self.iid_field]
        new_data[self.iid_field][pos_inter_num:] = neg_item_ids
        labels = torch.zeros(new_data_size)
        labels[:pos_inter_num] = 1.0
        new_data[self.label_field] = labels
        new_data = Interaction(new_data)
        return new_data
    ...
    def _next_batch_data(self):
        if self.neg_sample_args['strategy'] == 'by':
            ...
            for idx, uid in enumerate(uid_list):
                index = self.uid2index[uid]
                data_list.append(self._new_neg_sampling(self.dataset[index]))  # 对应！！！
                idx_list += [idx for i in range(self.uid2items_num[uid] + self.neg_sample_num)]
                positive_u += [idx for i in range(self.uid2items_num[uid])]
                positive_i = torch.cat((positive_i, self.dataset[index][self.iid_field]), 0)
