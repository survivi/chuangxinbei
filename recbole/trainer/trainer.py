...
class Trainer(AbstractTrainer):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        if 'dynamic' in self.config['train_neg_sample_args'].keys() and self.config['train_neg_sample_args'][
            'dynamic'] != 'none':
    ...
    def _neg_sample_batch_eval(self, batched_data):
        ...
        if batch_size <= self.test_batch_size:
            origin_scores = self.model.predict(interaction.to(self.device))  # 调用predict()
        else:
        ...
        if self.config['eval_type'] == EvaluatorType.VALUE:
            ...
            scores = torch.full((batch_user_num, self.tot_item_num), -np.inf, device=self.device).type(torch.LongTensor)
            scores[row_idx.type(torch.LongTensor), col_idx.type(torch.LongTensor)] = origin_scores.type(torch.LongTensor)
            return interaction, scores, positive_u, positive_i
...
class FairFedTrainer(Trainer):
    ...
    def _full_sort_batch_eval_basic(self, batched_data):
        ...
        try:
            # Note: interaction without item ids
            origin_scores = self.model.full_sort_predict(interaction.to(self.device))
        except NotImplementedError:
            inter_len = len(interaction)
            new_inter = interaction.to(self.device).repeat_interleave(self.tot_item_num)
            batch_size = len(new_inter)
    ...
    @torch.no_grad()
    def evaluate_basic(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        if not eval_data:
            return
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            df_dict[batch_idx] = pd.DataFrame(batch_arr, columns=['uid', 'iid', 'label', 'score'])
            self.eval_collector.eval_batch_collect(scores, interaction, positive_u, positive_i)
