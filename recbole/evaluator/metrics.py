...
class Recall(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)  # 每个用户的topk位置的item是否是positive|每个用户positive item数量
        result = self.metric_info(pos_index, pos_len)
...
class F1(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        P = Precision(self.config)
        p_dict = P.calculate_metric(dataobject)
        R = Recall(self.config)
        r_dict = R.calculate_metric(dataobject)
        metric_dict = {}
        for k in self.topk:
            key = 'f1@{}'.format(k)
            p = float(p_dict['precision@{}'.format(k)])
            r = float(r_dict['recall@{}'.format(k)])
            metric_dict[key] = 0 if p == 0 or r == 0 else round(2.0 * ((p * r) / (p + r)), 4)
        return metric_dict
